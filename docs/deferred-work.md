...
- **RESOLVED (2026-09-07, `spec-fix-vllm-version-checked-semantics.md`):** This item conflated two orthogonal concepts. `SUPPORTED_VLLM_RANGE` is the vLLM version range the app's *overhead calibration* targets (NFR16) — an app-wide, empirically-validated claim only the Epic 4 GPU harness can responsibly change. `vllm_version_checked` (per-preset) is provenance for when a preset was last checked against the project's own baseline vLLM version — it was never meant to record a given model's own minimum serving requirement. The real bug was that `glm-5.2.yaml` had been given `vllm_version_checked: "0.23"` using the wrong (per-model-minimum) semantic, inconsistent with every other preset's `"0.13"`. Fixed by correcting that one field back to `"0.13"` and clarifying the field's meaning in `models.py`'s `Field(description=...)` and `CONTRIBUTING.md` so this doesn't recur. `SUPPORTED_VLLM_RANGE` itself was left untouched — it still needs the (already-tracked, GPU-dependent) Epic 4 recalibration, not a manual bump. The "GLM-5.2 needs vLLM ≥0.23 to serve" fact itself wasn't lost — see the annotation on `spec-add-glm-5.2-preset.md`'s review order.
